...
@Getter
@Setter
public class NewsCardEditDialog extends AbstractDialog {
	...
	public NewsCardEditDialog(NewsCardResponseDto newsCardResponseDto) {
	...
	public void buildComponent() {
		...
		setHeaderTitle(DialogNameConstant.NEWS_CARD_EDIT_HEADER_TITLE);
		actionButton.setText(DialogNameConstant.NEWS_CARD_EDIT_ACTION_BUTTON_NAME);
		RoadmapImage.getImageUrl(RoadmapImage.CUSTOM_NEWS_CARD)
				.ifPresent(url -> image.setSrc(url));
		timePicker.setValue(LocalTime.from(newsCardResponseDto.getCreatedAt()));
		datePicker.setValue(LocalDate.from(newsCardResponseDto.getCreatedAt()));
		title.setValue(newsCardResponseDto.getTitle());
		description.setValue(newsCardResponseDto.getDescription());
