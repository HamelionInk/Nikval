...
public class RoadmapChapterController {
    ...
    @GetMapping
    public ResponseEntity<Page<RoadmapChapterResponseDto>> getAll(@ParameterObject @PageableDefault(sort = "id",
            direction = Sort.Direction.ASC, size = Integer.MAX_VALUE) Pageable pageable) {
        var responseBody = roadmapChapterService.getAll(pageable);
